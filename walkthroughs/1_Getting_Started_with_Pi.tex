...
\subsection{Add Connectivity Files}

\subsection{Add network configuration file to SD card for a secure shell (SSH) connection.} \label{networkssh}

\begin{enumerate}
  \item Add a ``ssh'' file to your boot partition on the SD card.\footnote{you'll need to define what this means}.
  \begin{itemize}
  \item Do this by creating an empty text file named \textbf{``ssh''}.\footnote{few students know how to do this}
  \item Make sure the file you create has no file extension.\footnote{Some computers don't show you the extension, but they are there! going to need to be more explicit.}
  \end{itemize}
  \item Create and add a file called \textbf{``wpa\_supplicant.conf''} to the boot partition\footnote{where is this?} on the SD card. This can be created with any text editor.
  \begin{itemize}
...
\begin{enumerate}
  ...
  \newline
  \url{https://www.realvnc.com/en/connect/download/viewer/}%\footnote{I don't think this will work -- I prefer using remote desktop connection... why did you pick this one?}
