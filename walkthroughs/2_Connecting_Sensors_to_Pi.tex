...
\begin{enumerate}
  ...
  \item Attach the PMS5003 adapter to the Breadboard -- B23-B30, where the B30 aligns with the 5 volt power.
  
  \item Connect 2 male-to-female connectors (the ones with the black rectangles on either end):
...
\begin{itemize}
  ...
  \item Connect the wire attached to the row corresponding to the RXD (recieve) terminal (d27) to TXD (transmit) PIN 8 on the Raspberry Pi. 
  \item Connect the wire attached to the TXD (transmit) terminal to RXD (recieve) PIN 10 on the Raspberry Pi.
