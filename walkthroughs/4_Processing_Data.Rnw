...
<<>>=
library(dplyr)
cleandata$group <- cut(cleandata$DateTime, breaks="30 min")

cleandata %>% group_by(cleandata$group) %>% summarize(mins_output = mean(cleandata$pm25_cf))
...
cleandata %>%
  ...
  summarize(Concentration = mean(pm25_cf))
...
<<>>=
str(cleandata)
dataset1 = subset(cleandata, 
  subset=
    DateTime<as.Date("2020-10-22 15:32:25") & 
    DateTime>as.Date("2020-10-21 14:35:28"))
dataset2 = subset(cleandata, 
  subset=
    DateTime<as.Date("2020-10-22 15:32:25") & 
    DateTime>as.Date("2020-10-21 14:35:28"))
@
...
<<>>=
t.test(dataset1$PM25, dataset2$PM25, paired=TRUE)
